-- Roles: Superusuario (ve y gestiona todo), Editor (recetas de todos, no ingredientes), Usuario (solo propias/públicas).
-- El rol se guarda en app_metadata.role en Supabase Auth: "superusuario" | "editor" | (por defecto) usuario.
-- Cómo dar rol: Authentication → Users → [usuario] → Edit → App Metadata → {"role": "superusuario"} o {"role": "editor"}
...
-- Función auxiliar: true si el usuario actual tiene rol "editor" (ve/edita todas las recetas, no gestiona ingredientes)
...
-- SELECT: superusuario y editor ven todas; usuario ve propias o públicas
...
-- UPDATE: superusuario, editor o dueño
CREATE POLICY "recetas_update"
  ON public.recetas FOR UPDATE TO authenticated
  USING (public.es_superusuario() OR public.es_editor() OR auth.uid() = user_id)
  WITH CHECK (public.es_superusuario() OR public.es_editor() OR auth.uid() = user_id);

-- DELETE: superusuario, editor o dueño
